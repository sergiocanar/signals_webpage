 ...
 * Genera un gráfico de línea de un taquigrama utilizando Highcharts.
 *
 * @param {number[]} timeArray - Array de tiempos correspondientes en segundos.
 */
function plotTachogram(timeArray) {
    Highcharts.chart('tachogram-container', {
        chart: { type: 'line' },
        title: { text: 'Taquigrama' },
        xAxis: { title: { text: 'Tiempo (segundos)' }, categories: timeArray },
        yAxis: { title: { text: 'Intervalo RR (segundos)' } },
        series: [{ name: 'Intervalo RR', data: rrIntervals }]
    });
}
...
function updatePlots(freq, startPoint, windowSize) {
    ...
    Highcharts.chart('ecg-container', {
        chart: { type: 'line' },
        title: { text: 'Señal ECG' },
        xAxis: { title: { text: 'Tiempo (segundos)' }, categories: timeWindow },
        yAxis: { title: { text: 'Amplitud' } },
        series: [{ name: 'Señal ECG', data: signalWindow }]
    });
...
function calculateBPM() {
    ...
    if (rrIntervals.length > 0) {
        ...
        if (bpm < 60) {
            alertView.innerText = "¡Alerta! posible bradicardia.";
            alertView.style.color = "red";
            moreInfoButton.onclick = function () {
                alert(`La bradicardia es una condición en la que el corazón late más lento de lo normal. Puede ser grave en algunos casos y puede requerir tratamiento.
            ...
            alertView.innerText = "¡Alerta! posible taquicardia.";
            alertView.style.color = "red";
            moreInfoButton.onclick = function () {
                alert(`La taquicardia es una condición en la que el corazón late más rápido de lo normal. Es importante monitorearla ya que puede llevar a problemas más serios.
                    
                    Taquicardia leve: 100-120 bpm.
                    
                    Taquicardia moderada: 120-150 bpm.
                    
                    Taquicardia severa: Más de 150 bpm.`);
            };
        } else {
            alertView.innerText = "Dentro del rango normal.";
            alertView.style.color = "green";
            moreInfoButton.onclick = function () {
                alert("Tu ritmo cardíaco está dentro de los niveles normales. ¡Sigue así!");
            };
        }
    } else {
        document.getElementById('bpm-result').innerText = "N/A";
        alertView.innerText = "No hay suficientes picos para calcular el BPM.";
